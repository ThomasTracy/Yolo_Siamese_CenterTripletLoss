...
class yolov3(object):
    ...
    def siamese_forward(self, input1, input2):
        ...
        inputs = tf.concat([input1, input2], 0)


        with slim.arg_scope([slim.conv2d, slim.batch_norm], reuse=False):
            with tf.variable_scope('encoder'):
                x = slim.conv2d(inputs, 64,
                            kernel_size=[5,5], padding='valid', scope='conv1')
                # x = slim.batch_norm(x, scope='bn1')
                x = tf.layers.batch_normalization(x, training=False)
                x = tf.nn.relu(x)
                x = slim.max_pool2d(x, [2,2])

                x = slim.conv2d(x, 128, kernel_size=[5,5], padding='valid', scope='conv2')
                x = tf.layers.batch_normalization(x, training=False)
                x = tf.nn.relu(x)
                x = slim.max_pool2d(x, [2,2])

                x = slim.conv2d(x, 256, kernel_size=[3, 3], padding='valid', scope='conv3')
                x = tf.layers.batch_normalization(x, training=False)
                x = tf.nn.relu(x)
                x = slim.max_pool2d(x, [2, 2])

                x = slim.conv2d(x, 512, kernel_size=[3, 3], padding='valid', scope='conv4')
                x = tf.layers.batch_normalization(x, training=False)
                x = tf.nn.relu(x)

                x = slim.flatten(x)

        output1 = x[:batch_size]
        output2 = x[batch_size:]

        # define the loss
        distance = tf.pow((output2 - output1), 2)  # Squared distance
        with tf.variable_scope('dense'):
            score = slim.fully_connected(distance, num_outputs=1)

        labels_predict = tf.cast(score > 0, tf.float32)
        labels_predict = tf.squeeze(labels_predict)
        score = tf.squeeze(score)

        return labels_predict, score, x